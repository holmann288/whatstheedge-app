export default function Home() {
  const signals = [
    { game: "Lakers vs Celtics", pick: "Celtics -3.5", edge: "+5.2%", clv: "+2.1%", confidence: "HIGH", sport: "NBA" },
    { game: "Duke vs UNC", pick: "Over 152.5", edge: "+4.8%", clv: "+1.9%", confidence: "HIGH", sport: "NCAAB" },
    { game: "Gonzaga vs Saint Mary's", pick: "Gonzaga -6", edge: "+3.1%", clv: "+0.8%", confidence: "MED", sport: "NCAAB" },
  ];
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Active Signals", value: "3", positive: true },
          ].map((stat) => (
        ...
        <div>
          ...
          <div className="space-y-3">
            {signals.map((s, i) => (
              <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-lg p-5 flex items-center justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded">{s.sport}</span>
                    <span className="text-sm text-zinc-300">{s.game}</span>
                  </div>
                  <div className="text-white font-bold text-lg">{s.pick}</div>
                </div>
                <div className="text-right space-y-1">
                  <div className="text-green-400 font-bold text-lg">{s.edge}</div>
        ...
        <div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 space-y-3 text-sm text-zinc-300 leading-relaxed">
            <p>3 high-confidence signals identified across NBA and NCAAB markets. Model MAE holding at 10.65 — within healthy range. CLV tracker showing positive results across 65% of resolved bets.</p>
